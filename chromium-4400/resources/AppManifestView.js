...
export class AppManifestView extends UI.Widget.VBox {
    ...
    async _appendImageResourceToSection(baseUrl, imageResource, section, isScreenshot) {
        ...
        if (!imageResource.sizes) {
            ...
            const [width, height] = imageResource.sizes.split('x').map(/** @param {*} x*/ /** @param {*} x*/ x => parseInt(x, 10));
            if (!isScreenshot && (width !== height)) {
                ...
                imageResourceErrors.push(ls `Actual size (${image.naturalWidth}×${image.naturalHeight})px of ${resourceName} ${imageUrl} does not match specified size (${width}×${height}px)`);
            }
            else if (image.naturalWidth !== width) {
                imageResourceErrors.push(ls `Actual width (${image.naturalWidth}px) of ${resourceName} ${imageUrl} does not match specified width (${width}px)`);
            }
            else if (image.naturalHeight !== height) {
                imageResourceErrors.push(ls `Actual height (${image.naturalHeight}px) of ${resourceName} ${imageUrl} does not match specified height (${height}px)`);
            }
