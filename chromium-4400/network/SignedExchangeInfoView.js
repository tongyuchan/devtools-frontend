...
export class SignedExchangeInfoView extends UI.Widget.VBox {
    ...
    constructor(request) {
        ...
        const learnMoreNode = UI.XLink.XLink.create('https://github.com/WICG/webpackage', Common.UIString.UIString('Learn\xa0more'), 'header-toggle');
        ...
        if (signedExchangeInfo.header) {
            ...
            headerCategory.createLeaf(this._formatHeader(Common.UIString.UIString('Response code'), String(header.responseCode)));
            headerCategory.createLeaf(this._formatHeader(Common.UIString.UIString('Header integrity hash'), header.headerIntegrity));
            ...
            for (let i = 0; i < header.signatures.length; ++i) {
                ...
                signatureCategory.createLeaf(this._formatHeaderForHexData(Common.UIString.UIString('Signature'), signature.signature, errorFieldSet.has(Protocol.Network.SignedExchangeErrorField.SignatureSig)));
                if (signature.certUrl) {
                    const certURLElement = this._formatHeader(Common.UIString.UIString('Certificate URL'), signature.certUrl, errorFieldSet.has(Protocol.Network.SignedExchangeErrorField.SignatureCertUrl));
                ...
                signatureCategory.createLeaf(this._formatHeader(Common.UIString.UIString('Integrity'), signature.integrity, errorFieldSet.has(Protocol.Network.SignedExchangeErrorField.SignatureIntegrity)));
                if (signature.certSha256) {
                    signatureCategory.createLeaf(this._formatHeaderForHexData(Common.UIString.UIString('Certificate SHA256'), signature.certSha256, errorFieldSet.has(Protocol.Network.SignedExchangeErrorField.SignatureCertSha256)));
                }
                signatureCategory.createLeaf(this._formatHeader(Common.UIString.UIString('Validity URL'), signature.validityUrl, errorFieldSet.has(Protocol.Network.SignedExchangeErrorField.SignatureValidityUrl)));
                signatureCategory.createLeaf().title = this._formatHeader(Common.UIString.UIString('Date'), new Date(1000 * signature.date).toUTCString(), errorFieldSet.has(Protocol.Network.SignedExchangeErrorField.SignatureTimestamps));
                signatureCategory.createLeaf().title = this._formatHeader(Common.UIString.UIString('Expires'), new Date(1000 * signature.expires).toUTCString(), errorFieldSet.has(Protocol.Network.SignedExchangeErrorField.SignatureTimestamps));
            }
        }
        if (signedExchangeInfo.securityDetails) {
            ...
            securityCategory.createLeaf(this._formatHeader(Common.UIString.UIString('Valid from'), new Date(1000 * securityDetails.validFrom).toUTCString()));
            securityCategory.createLeaf(this._formatHeader(Common.UIString.UIString('Valid until'), new Date(1000 * securityDetails.validTo).toUTCString()));
