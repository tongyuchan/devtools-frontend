...
export class ServiceWorkerCacheModel extends SDKModel {
    ...
    async deleteCacheEntry(cache, request) {
        ...
        if (response.getError()) {
            Common.Console.Console.instance().error(Common.UIString.UIString('ServiceWorkerCacheAgent error deleting cache entry %s in cache: %s', cache.toString(), response.getError()));
            return;
        }
    }
    /**
