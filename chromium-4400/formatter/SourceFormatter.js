...
/** @type {!WeakMap<!Object, !SourceFormatData>} */
        ...
        const resultPromise = new Promise(async (resolve) => {
            ...
            FormatterInterface.format(uiSourceCode.contentType(), uiSourceCode.mimeType(), content || '', async (formattedContent, formatterMapping) => {
                ...
                for (const decoration of uiSourceCode.allDecorations()) {
                    ...
                    formattedUISourceCode.addDecoration(new TextUtils.TextRange.TextRange(startLocation[0], startLocation[1], endLocation[0], endLocation[1]), 
...
class ScriptMapping {
    ...
    uiLocationToRawLocations(uiSourceCode, lineNumber, columnNumber) {
        ...
        if (formatData.originalSourceCode.contentType() === Common.ResourceType.resourceTypes.Document) {
            ...
            if (debuggerModel) {
                ...
                const locations = /** @type {!Array<!SDK.DebuggerModel.Location>} */ (scripts.map(script => script.rawLocation(originalLine, originalColumn)).filter(l => Boolean(l)));
    ...
    _scriptsForUISourceCode(uiSourceCode) {
        ...
        if (uiSourceCode.contentType().isScript()) {
            ...
            return /** @type {!Array<!SDK.Script.Script>} */ (rawLocations.map(location => location.script()).filter(script => Boolean(script)));
        }
        return [];
    }
}
/** @type {!WeakMap<!Workspace.UISourceCode.UISourceCode, !Array<!SDK.CSSStyleSheetHeader.CSSStyleSheetHeader>>} */
...
class StyleMapping {
    ...
    _headersForUISourceCode(uiSourceCode) {
        if (uiSourceCode.contentType() === Common.ResourceType.resourceTypes.Document) {
            ...
            return /** @type {!Array<!SDK.CSSStyleSheetHeader.CSSStyleSheetHeader>} */ (rawLocations.map(rawLocation => rawLocation.header()).filter(header => Boolean(header)));
        }
        return [];
    }
}
