...
export class FormatterWorkerPool {
    ...
    javaScriptOutline(content, callback) {
        this._runChunkedTask("javaScriptOutline" /* JAVASCRIPT_OUTLINE */, { content: content }, onDataChunk);
        /**
         * @param {boolean} isLastChunk
         * @param {*} data
         */
    ...
    outlineForMimetype(content, mimeType, callback) {
        ...
        function javaScriptCallback(isLastChunk, items) {
            callback(isLastChunk, items.map(item => ({ line: item.line, column: item.column, title: item.name, subtitle: item.arguments })));
        }
        /**
         * @param {boolean} isLastChunk
    ...
    findLastExpression(content) {
        return /** @type {!Promise<?string>} */ (this._runTask("findLastExpression" /* FIND_LAST_EXPRESSION */, { content }));
    }
    /**
     * @param {string} content
    ...
    findLastFunctionCall(content) {
        return /** @type {!Promise<?{baseExpression: string, receiver: string, argumentIndex: number, functionName: string}>} */ (this._runTask("findLastFunctionCall" /* FIND_LAST_FUNCTION_CALL */, { content }));
    }
    /**
     * @param {string} content
    ...
    argumentsList(content) {
        return /** @type {!Promise<!Array<string>>} */ (this._runTask("argumentsList" /* ARGUMENTS_LIST */, { content }));
    }
}
class Task {
    /**
