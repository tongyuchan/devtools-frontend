...
export class SensorsView extends UI.Widget.VBox {
    ...
    _setDeviceOrientation(deviceOrientation, modificationSource) {
        ...
        UI.ARIAUtils.alert(ls `Device orientation set to alpha: ${deviceOrientation.alpha}, beta: ${deviceOrientation.beta}, gamma: ${deviceOrientation.gamma}`, this._orientationSelectElement);
    }
    /**
    ...
    _createDeviceOrientationOverrideElement(deviceOrientation) {
        ...
        this._alphaSetter = this._createAxisInput(cellElement, this._alphaElement, Common.UIString.UIString('\u03B1 (alpha)'), SDK.EmulationModel.DeviceOrientation.alphaAngleValidator);
        ...
        this._betaSetter = this._createAxisInput(cellElement, this._betaElement, Common.UIString.UIString('\u03B2 (beta)'), SDK.EmulationModel.DeviceOrientation.betaAngleValidator);
        ...
        this._gammaSetter = this._createAxisInput(cellElement, this._gammaElement, Common.UIString.UIString('\u03B3 (gamma)'), SDK.EmulationModel.DeviceOrientation.gammaAngleValidator);
        this._gammaSetter(String(deviceOrientation.gamma));
        const resetButton = UI.UIUtils.createTextButton(Common.UIString.UIString('Reset'), this._resetDeviceOrientation.bind(this), 'orientation-reset-button');
    ...
    _appendTouchControl() {
        ...
        const control = UI.SettingsUI.createControlForSetting(Common.Settings.Settings.instance().moduleSetting('emulation.touch'), ls `Forces touch instead of click`);
        if (control) {
    ...
    _appendIdleEmulator() {
        ...
        const control = UI.SettingsUI.createControlForSetting(Common.Settings.Settings.instance().moduleSetting('emulation.idleDetection'), ls `Forces selected idle state emulation`);
        if (control) {
            container.appendChild(control);
        }
    }
