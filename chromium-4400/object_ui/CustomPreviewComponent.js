...
export class CustomPreviewSection {
    ...
    async _loadBody() {
        ...
        if (customPreview.bodyGetterId) {
            const bodyJsonML = await this._object.callFunctionJSON(bodyGetter => /** @type {function():*} */ (bodyGetter)(), [{ objectId: customPreview.bodyGetterId }]);
            if (!bodyJsonML) {
                return;
            }
...
export class CustomPreviewComponent {
    ...
    _contextMenuEventFired(event) {
        ...
        if (this._customPreviewSection) {
            contextMenu.revealSection().appendItem(Common.UIString.UIString('Show as JavaScript object'), this._disassemble.bind(this));
        }
